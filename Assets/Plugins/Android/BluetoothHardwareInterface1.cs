...
public class BluetoothLEHardwareInterface1
{
	...
	private static BluetoothDevice bluetoothDeviceScript;
...
//		Debug.Log(message);
	...
	public static BluetoothDevice Initialize (bool asCentral, bool asPeripheral, Action action, Action<string> errorAction)
	{
		...
		if (bluetoothLEReceiver != null)
		{
			bluetoothDeviceScript = bluetoothLEReceiver.GetComponent<BluetoothDevice> ();
			if (bluetoothDeviceScript == null)
				bluetoothDeviceScript = bluetoothLEReceiver.AddComponent<BluetoothDevice> ();
		...
		BluetoothLEHardwareInterface1.OSXBluetoothLEInitialize (asCentral, asPeripheral);
#else
        if (Application.isEditor)
		{
			if (bluetoothDeviceScript != null)
...
#if EXPERIMENTAL_MACOS_EDITOR && (UNITY_EDITOR_OSX || UNITY_STANDALONE_OSX)
		BluetoothLEHardwareInterface1.OSXBluetoothLEDeInitialize ();
#else
        if (Application.isEditor)
		{
			if (bluetoothDeviceScript != null)
