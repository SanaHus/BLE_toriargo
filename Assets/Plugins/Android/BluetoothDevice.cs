...
public class BluetoothDevice : MonoBehaviour
{
	...
	public Action<BluetoothLEHardwareInterface1.iBeaconData> DiscoveredBeaconAction;
	...
	public void OnBluetoothMessage (string message)
	{
		if (message != null)
		{
            ...
            for (int i = 0; i < parts.Length; ++i)
                BluetoothLEHardwareInterface1.Log(string.Format("Part: {0} - {1}", i, parts[i]));
			...
			{
				BluetoothLEHardwareInterface1.Log (parts[1]);
			}
			...
			{
				BluetoothLEHardwareInterface1.FinishDeInitialize ();
			...
			{
				BluetoothLEHardwareInterface1.Log ("Started Advertising");
			...
			{
				BluetoothLEHardwareInterface1.Log ("Stopped Advertising");
			...
			else if (message.Length >= deviceDiscoveredBeacon.Length && message.Substring (0, deviceDiscoveredBeacon.Length) == deviceDiscoveredBeacon)
			{
				if (parts.Length >= 7)
				{
					var iBeaconData = new BluetoothLEHardwareInterface1.iBeaconData ();
					...
					if (!int.TryParse (parts[6], out iOSProximity))
						iOSProximity = 0;
					iBeaconData.iOSProximity = (BluetoothLEHardwareInterface1.iOSProximity)iOSProximity;
	...
	public void OnBluetoothData (string deviceAddress, string characteristic, string base64Data)
	{
		if (base64Data != null)
		{
			...
			if (bytes.Length > 0)
			{
                ...
                BluetoothLEHardwareInterface1.Log ("Device: " + deviceAddress + " Characteristic Received: " + characteristic);

				string byteString = "";
				foreach (byte b in bytes)
					byteString += string.Format ("{0:X2}", b);

				BluetoothLEHardwareInterface1.Log (byteString);
	...
	public void OnPeripheralData (string characteristic, string base64Data)
	{
		if (base64Data != null)
		{
			...
			if (bytes.Length > 0)
			{
				BluetoothLEHardwareInterface1.Log ("Peripheral Received: " + characteristic);
